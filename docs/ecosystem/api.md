...
Sending an API request to Centrifugo is a simple task to do in any programming language - this is just a POST request with JSON payload in body and `Authorization` header. See more in [special chapter](../server/http_api) in server section.
...
Also, keep in mind that Centrifugo [has GRPC API](../server/grpc_api) so you can automatically generate client API code for your language.
