...
Combined with the [PostgreSQL stream broker](#postgresql-broker) and/or the [PostgreSQL map broker](./map_subscriptions.md#postgresql), this completes the "Centrifugo + PostgreSQL, no Redis" story for multi-node deployments — a fully functional cluster running on PostgreSQL alone.
...
:::tip Splitting controller load from broker load

[Centrifugo PRO](../pro/scalability.md#setting-custom-controller) extends the controller story with Redis and Nats custom controllers, letting you run channel traffic over one backend and controller traffic over another for isolation.
