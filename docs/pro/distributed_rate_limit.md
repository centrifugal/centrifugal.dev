...
In addition to connection operation rate limiting features, Centrifugo PRO provides a generic high-precision rate limiting API. It may be used for custom quota management tasks not even related to real-time connections. Its distributed nature allows managing quotas across different instances of your application backend.
...
Centrifugo distributed rate limiting is a high-performance, zero-configuration, Redis-based token bucket with millisecond precision. Zero configuration in this case means that you don't have to preconfigure buckets in Centrifugo – bucket configuration is part of the request to check allowed limits.
...
Centrifugo PRO also returns `allowed_in` and `server_time` fields to help understand when the action will be allowed. These two fields are only appended when `tokens_left` is less than the requested `score`. `allowed_in` + `server_time` will provide you a timestamp in the future (in milliseconds) when the action can be executed. So you can delay the next action execution until that time if possible.
...
Note that, just like most other features in Centrifugo, it's possible to configure Redis shards here or use Redis Cluster.
