...
Then use it directly with the `X-API-Key` header:

```
curl -N -X POST http://localhost:8000/admin/trace \
  -H "X-API-Key: <your-secret-key>" \
  -d '{"type": "user", "entity": "56"}'
```

The `-N` flag disables output buffering so events are printed in real-time. To save output to a file, add `-o trace.txt` (the file will be written to continuously until the connection is closed with Ctrl+C).
...
```
curl -N -X POST http://localhost:8000/admin/trace -H "Authorization: token <ADMIN_AUTH_TOKEN>" -d '{"type": "user", "entity": "56"}'
```
