...
function convert(sourceObj, pathMap) {
    ...
    if (newConfig.prometheus && newConfig.prometheus.enabled) {
        if (newConfig.prometheus.channel_namespace_for_transport_messages_sent || newConfig.prometheus.channel_namespace_for_transport_messages_received) {
            newConfig.prometheus.channel_namespace_resolution = true;
    ...
    // if newConfig.push_notifications.max_inactive_device_days set then transform it to newConfig.push_notifications.max_inactive_device_interval,
    // but it was previously set to days, so we need to multiply it by 24 and add "h" suffix and convert to string.
    if (newConfig.push_notifications && newConfig.push_notifications.max_inactive_device_days) {
