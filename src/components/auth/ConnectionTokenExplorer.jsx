...
import { decideConnect, buildTokenPayload, buildConnectConfig } from './authtoken';
import { Snippet, JsonBlock } from './ui';
import styles from './styles.module.css';

// Generic OSS connection-JWT explorer: build the claims, see whether the
// connection is accepted (claim-level checks) and what it becomes, plus the
// token payload to sign and the token config.
...
export default function ConnectionTokenExplorer() {
  ...
  const result = useMemo(() => decideConnect(state), [state]);
  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.caveat}>
        Models Centrifugo OSS connection-token claim checks and the resulting connection. Signature verification
        (HMAC/RSA/ECDSA/JWKS) needs your key and is not simulated here. <code>exp</code> values are shown as example timestamps.
      </div>
    </div>
  );
}
